...
/**
 * @brief Commands Cybot to move forward certain distances and tracks the distance of Cybot had moved
 *
 * @author Kai Heng Gan
 *
 * @param sensor
 * @param cm
 */
void oi_moveForward(oi_t*sensor, float cm){
    int count = 0;
    float mm = cm*10;
    char bumppedLeft[] = "Bumpped Left";
    char bumppedRight[] = "Bumpped Right";
    while(count<mm){
        oi_update(sensor);
        // When the collision occurs on the left bumper
         if(sensor->bumpLeft){
             uart_sendStr(bumppedLeft);
             break;
         }

         else if(sensor->bumpRight){
             uart_sendStr(bumppedRight);
             break;
          }

         // When the collision occurs on the right bumper
         if(sensor->cliffFrontLeft){
             uart_sendStr("Sensed hole on the front left");
             break;
         }
         if(sensor->cliffLeft){
             uart_sendStr("Sensed hole on the left");
             break;
         }
         if(sensor->cliffFrontRight){
             uart_sendStr("Sensed hole on the front right");
             break;
         }
         if(sensor->cliffRight){
             uart_sendStr("Sensed hole on the right");
             break;
         }

         if(sensor->cliffFrontLeftSignal >=2600 || sensor->cliffFrontRightSignal >= 2600){
             uart_sendStr("White tape Detected at the front. STOP!");
             break;
         }

         if(sensor->cliffLeftSignal >=2600){
             uart_sendStr("White tape Detected on the LEFT. STOP!");
             break;
         }
         if(sensor->cliffRightSignal >=2600){
             uart_sendStr("White tape Detected on the RIGHT. STOP!");
             break;
         }

        count += sensor->distance;
    }
    timer_waitMillis(100);
}

/**
 * @brief Commands Cybot to move backward certain distances and tracks the distance of Cybot had moved
 *
 * @author Kai Heng Gan
 *
 * @param sensor
 * @param cm
 */
void oi_moveBackward(oi_t*sensor, float cm){
    float sum = 0;
    float mm = cm*10;
    while(sum>mm){
         oi_update(sensor);
...
/**
 * @brief Commands Cybot to turn certain degree (Counterclockwise) and tracks the degree of Cybot had turned
 *
 * @author Kai Heng Gan
 *
 * @param sensor
 * @param deg
 */
void oi_turnCounterClockwise(oi_t*sensor, float deg){
    volatile float sum_angle = 0;
    while(sum_angle<deg){
         oi_update(sensor);
         sum_angle += sensor->angle;
    }
}

/**
 * @brief Commands Cybot to turn certain degree (Clockwise) and tracks the degree of Cybot had turned
 *
 * @author Kai Heng Gan
 *
 * @param sensor
 * @param deg
 */
void oi_turnClockwise(oi_t*sensor, float deg){
    volatile float sum_angle = 0;
    while(sum_angle>deg){
        oi_update(sensor);
        sum_angle += sensor->angle;
